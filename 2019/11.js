...
const program = new Program(puzzleInput)
...
const turn = (dir, change) => {
	if (change === 0) return (dir + 3) % 4
	if (change === 1) return (dir + 1) % 4
}

const move = (position, dir) => {
	switch(dir) {
		case 0:
			return position.add(new Coord('0_1'))
		case 1:
			return position.add(new Coord('1_0'))
		case 2:
			return position.add(new Coord('0_-1'))
		case 3:
			return position.add(new Coord('-1_0'))
	}
}
...
// changed to PART 2
const run = program.runSteps(BLACK)
const map = {'0_0': 1}
...
while (true) {
	const panel = map[position]

	res = run.next(panel ? panel : BLACK)
	// paint
	map[position] = res.value.stdout[0]
	mapMeta.minX = Math.min(position.x, mapMeta.minX)
	mapMeta.minY = Math.min(position.y, mapMeta.minY)
	mapMeta.maxX = Math.max(position.x, mapMeta.maxX)
	mapMeta.maxY = Math.max(position.y, mapMeta.maxY)
	if (res.done) break

	res = run.next()
	// turn and move
	dir = turn(dir, res.value.stdout[1])
...
assert.strictEqual(Object.keys(map).length, 1934)
